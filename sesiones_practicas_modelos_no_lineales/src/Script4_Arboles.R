...
# Definir aqui la funcion  
	...
	modelos <- lapply(c(1:num_trees), function(x) { 
		mod <- randomForest(formula, data=datos , ntree=x)
		mod
	} )
	
	tasa_acierto <- lapply(modelos, function(x) {
		yhat.bag = predict (x ,newdata =datos)
		resul = as.data.frame(cbind(predicted = yhat.bag, y=datos[,ncol(datos)]))
		sol <- acierto(resul)
		sol
	})
	
	plot(c(1:num_trees), tasa_acierto)
